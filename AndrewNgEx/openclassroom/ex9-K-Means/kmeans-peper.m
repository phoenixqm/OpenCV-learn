...
% Load small image
A = double(imread('pepers.jpg'));

dim1 = size(A,1); % number of pixels in picture's length/width
dim2 = size(A,2); % number of pixels in picture's length/width
...
imshow(uint8(round(A))); hold off

% Save image
imwrite(uint8(round(A)), 'pepers_kmeans.jpg');
