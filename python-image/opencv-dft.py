...
if __name__ == "__main__":
    ...
    cv2.dft(dft_A, dst=dft_A, flags = cv2.DFT_COMPLEX_OUTPUT, nonzeroRows=h)

    dft_shift = np.fft.fftshift(dft_A)

    cv2.imshow("input gray", im)

    # Split fourier into real and imaginary parts
    image_Re, image_Im = cv2.split(dft_shift)
    # cv2.imshow("real", image_Re)
    # cv2.imshow("imaginary", image_Im)
