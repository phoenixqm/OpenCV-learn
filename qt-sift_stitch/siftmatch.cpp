...
void SiftMatch::on_mosaicButton_clicked()
{
    ...
    if(H)
    {
        ...
        cvSaveImage(name_xformed.replace( name_xformed.lastIndexOf(".",-1)-1 , 1 , "_Mosaic").toLatin1().data() , xformed_simple);//保存简易拼接图
        cvSaveImage(name_xformed.insert( name_xformed.lastIndexOf(".",-1) , "_Proc").toLatin1().data() , xformed_proc);//保存处理后的拼接图
