function model = optimize(model_shape, gradient_function, ...
                            training_data, learning_rate, n_iterations)
...
% that we saw in PA3, which we were minimizing. The returned gradient is 
% an array of the same shape as the provided <model> parameter.
...
% This returns the matrix of weights of the trained model.
    ...
    for iteration_number = 1:n_iterations,
        mini_batch = extract_mini_batch(training_data, start, mini_batch_size);
        start = mod(start + mini_batch_size, size(training_data.inputs, 2));
