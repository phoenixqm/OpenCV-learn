...
function a4_main(n_hid, lr_rbm, lr_classification, n_iterations)
    ...
    rbm_w = optimize([n_hid, 256], ...          % discard labels
                     @(rbm_w, data) cd1_improved(rbm_w, data.inputs), ...  
                     data_sets.training, ...
                     lr_rbm, ...
                     n_iterations);
    ...
    hid_to_class = optimize([10, n_hid], ...
                    @(model, data) classification_phi_gradient(model, data), ...
                    data_2, lr_classification, n_iterations);
    % report results
    for data_details = reshape({'training',     data_sets.training, ...
        ...
        % log of probability of each class. size: 
        % <number of classes, i.e. 10> by <number of data cases>
        log_class_prob = class_input - repmat(class_normalizer, [size(class_input, 1), 1]); 
        
        Er = argmax_over_rows(class_input) ~= argmax_over_rows(data.targets);
        error_rate = mean(double(Er)); % scalar.
        ...
        fprintf(['For the %s data, the classification cross-entropy loss is %f,' ... 
            'and the classification error rate (i.e. the misclassification rate)' ... 
...
% This is about a very simple model: there's an input layer, and a softmax 
%  output layer. There are no hidden layers, and no biases.
% This returns the gradient of phi (a.k.a. negative the loss) for the 
% <input_to_class> matrix.
...
% first: forward pass
    ...
    % size: <number of classes> by <number of data cases>
    d_loss_by_d_class_input = -(data.targets - class_prob) ./ size(data.inputs, 2); 
    
    % size: <number of classes> by <number of input units>
    d_loss_by_d_input_to_class = d_loss_by_d_class_input * data.inputs.';
